...
from typing import Any, List, Hashable, Union, Set, Optional, Iterator
import pprint
...
def test():
    ...
    for item in cfg.terms | cfg.rules:
        print("FIRST({}) = {}".format(item if item in cfg.rules else repr(item), cfg.first(item)))

    for rule in cfg.rules:
        print("FOLLOW({}) = {}".format(rule, cfg.follow(rule)))

    for prod in cfg.productions:
        print("SELECT({}) = {}".format(prod, cfg.select(prod)))
